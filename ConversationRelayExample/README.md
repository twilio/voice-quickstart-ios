...
2. [The Demo](#2-the-demo)  
   ...
   2.3. [Running the iOS client](#23-running-the-ios-client)  
...
1. **The Demo**  
   ...
   \- Configure the iOS client to connect to the Conversation Relay endpoint.
...
The first part of the demo uses Twilio Serverless functions and assets to provide a TwiML application for your client. If you have deployed it before, you will need to deploy it again to ensure that the new function/service is added (ai-conversation). 

Before deploying, create a .env file in `servers/twilio-serverless/` by copying from `servers/twilio-serverless/.env.example`
...
Update `servers/twilio-serverless/.env` with your Account SID, auth token, API Key and secret.
    ...
    API_KEY_SID=SKxxxx
    API_SECRET=xxxxxx
    APP_SID=APxxxx(available in step 2.2.5)
...
For more information look at the root [README.md](../README.md#bullet2) under the section  ***Use Twilio CLI to deploy access token and TwiML application to Twilio Serverless***

That README explains how to:

\- Install and configure `twilio-cli`.  
\- Deploy the Serverless project.  
\- Retrieve URLs and credentials needed by the iOS client.
...
Next, we need to create a TwiML application. A TwiML application identifies a public URL for retrieving [TwiML call control instructions](https://www.twilio.com/docs/voice/twiml). When your quickstart app makes a call to the Twilio cloud, Twilio will make a webhook request to this URL, your application server will respond with generated TwiML, and Twilio will execute the instructions you've provided.
...
### 2.3. Running the iOS client

To run the iOS client with Conversation Relay support, configure two variables in the main activity of the iOS app:

#### 2.3.1 Access Token

Open `ConversationRelayExample/ViewController.swift` and set the `accessToken` variable to a valid Twilio Access Token. This token is usually provided by using the twilio-cli tool to generate a token.
...
More information can be found at the root [README.md](../README.md#bullet4) under the section ***Generate an access token for the quickstart***.

#### 2.3.2 Conversation Relay URL

In the same file (`ConversationRelayExample/ViewController.swift`), set the `conversationRelayUrl` variable to the same public server URL used in the .env file, including the WebSocket path. For example:
   `wss://abc123.ngrok.app/conversation-relay`

Once those two values are in place, build and run the iOS app. When a call is established and the `<ConversationRelay>` TwiML is invoked, the client and server will communicate via the Conversation Relay WebSocket endpoint.
